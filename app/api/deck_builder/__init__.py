...
def deck_build(
    ...
    total_lands: Optional[int] = 34,
    total_creatures: Optional[int] = 35,
    total_artifacts: Optional[int] = 6,
    total_sorcery: Optional[int] = 7,
    total_enchantments: Optional[int] = 6,
    total_walkers: Optional[int] = 1,
    total_instants: Optional[int] = 9,
    basic_land_set: Optional[str] = "LRW",
):
    """
    ...
    # get creatures
    if total_creatures:
        deck_list = get_creatures(
            subtypes, total_creatures, abilities, deck_list, color_identity
        )

    # get artifacts
    if total_artifacts:
        deck_list = get_spells(
            spell_type="Artifact",
            subtypes=subtypes,
            num_cards=total_artifacts,
            abilities=abilities,
            deck_list=deck_list,
    ...
    if total_walkers:
        deck_list = get_spells(
            spell_type="Planeswalker",
            subtypes=subtypes,
            num_cards=total_walkers,
            abilities=abilities,
            deck_list=deck_list,
            color_identity=color_identity,
        )
    ...
    if total_lands:
        deck_list = get_spells(
            spell_type="Land",
            subtypes=subtypes,
            num_cards=total_lands,
            abilities=abilities,
            deck_list=deck_list,
            color_identity=color_identity,
            land_set=basic_land_set,
        )
